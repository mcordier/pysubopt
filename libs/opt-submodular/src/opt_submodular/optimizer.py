...
class GreedySubmodularOptimizer(SubmodularOptimizer[T]):
    ...
    def _get_optimal_subset(self, full_set: List[T]) -> Result:
        G = []
        lowest_cost = np.min([self.cost_fun([u]) for u in full_set])

        cost = 0

        if self.is_lazy:
            deltas = [self.fun([u]) / (self.cost_fun([u])) ** self.r for u in full_set]

        # stop when the cost is 90% of budget
        while len(full_set) != 0 and self.budget - cost >= 0.1 * self.budget:  
            if self.is_lazy:
                max_index = np.argmax(deltas)
                delta = (self.fun(G + [full_set[max_index]]) - self.fun(G)) / self.cost_fun([full_set[max_index]]) ** self.r
                ...
                L = [(self.fun(G + [u]) - self.fun(G)) / (self.cost_fun([u])) ** self.r for u in full_set]
        ...
        L = [self.fun([u]) for u in full_set if self.cost_fun([u]) < self.budget]
        v = np.array(L).argmax()
